...
export default function Navigation() {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState({ code: 'en', name: 'English', flag: 'us' });
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (isLanguageMenuOpen && !target.closest('[data-menu="language"]')) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-1.5">
            ...
            {/* Language Selector */}
            <div className="relative mr-1" data-menu="language">
              <button 
                onClick={() => setIsLanguageMenuOpen(!isLanguageMenuOpen)}
                className={`flex items-center space-x-1.5 px-3 py-2 rounded-md transition-all duration-200 text-base ${isLanguageMenuOpen ? 'bg-foreground/10 shadow-sm' : 'hover:bg-foreground/5'}`}
                aria-expanded={isLanguageMenuOpen}
                aria-haspopup="true"
              >
                <div className="flex items-center justify-center w-7 h-5 text-foreground/80">
                  <Image 
                    src={`/images/flags/${selectedLanguage.flag.toLowerCase()}.webp`} 
                    alt={selectedLanguage.name} 
                    width={24}
                    height={16}
                    className="w-auto h-auto max-h-full max-w-full rounded-none"
                    style={{ borderRadius: 0 }}
                  />
                </div>
                <span className="text-base text-foreground/80">{selectedLanguage.code.toUpperCase()}</span>
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 text-foreground/60 transition-transform duration-200 ${isLanguageMenuOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              
              {isLanguageMenuOpen && (
                <div className="absolute top-full left-0 mt-1 py-2 bg-white dark:bg-gray-950 rounded-md shadow-lg ring-1 ring-border/50 border border-gray-200 dark:border-gray-800 focus:outline-none z-50 max-h-60 overflow-y-auto min-w-[200px] w-max">
                  {languages.map((language) => (
                    <button
                      key={language.code}
                      className={`flex items-center w-full px-4 py-2 text-left transition-colors duration-150 ${
                        language.code === 'en' 
                          ? (selectedLanguage.code === language.code ? 'bg-primary/5 text-primary dark:text-primary-400 hover:bg-primary/10 dark:hover:bg-primary-900/20' : 'text-gray-800 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800')
                    ...
                    >
                      <div className="flex items-center justify-center w-8 h-6 mr-3">
                        <Image 
                          src={`/images/flags/${language.flag.toLowerCase()}.webp`} 
                          alt={language.name} 
                          width={24}
                          height={16}
                          className="w-auto h-auto max-h-full max-w-full rounded-none"
            ...
            <div className="relative ml-2" data-menu="user">
              {isAuthenticated ? (
                  ...
                  {isUserMenuOpen && (
                    <div className="absolute top-full right-0 mt-2 py-2 rounded-md shadow-lg ring-1 ring-border/10 focus:outline-none z-50 min-w-[280px] w-max bg-white dark:bg-slate-800 text-gray-800 dark:text-white border border-gray-200 dark:border-slate-700 user-menu-dropdown">
                      ...
                      <div className="p-2 border-b border-border/30 dark:border-gray-700/50">
                        <p className="px-3 py-1 text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Account</p>
                        <Link
                          href="/user/profile"
                        ...
                        >
                          ...
                          <span className="font-medium text-gray-800 dark:text-white">Profile Settings</span>
                        </Link>
                        <Link
                          href="/user/stories"
                          className="flex items-center w-full px-3 py-2 text-left hover:bg-foreground/10 dark:hover:bg-gray-800 rounded transition-colors duration-200 text-gray-800 dark:text-white group"
                        ...
                        >
                          ...
                          <span className="font-medium text-gray-800 dark:text-white">My Stories</span>
                        </Link>
                      </div>
                      ...
                      {user?.isAdmin && (
                        <div className="p-2 border-b border-border/30 dark:border-gray-700/50">
                          <p className="px-3 py-1 text-xs font-semibold text-amber-600 dark:text-amber-400 uppercase tracking-wider">Admin Controls</p>
                          <Link
                            href="/admin/stories"
                          ...
                          >
                            ...
                            <span className="font-medium text-amber-800 dark:text-amber-200">Stories</span>
                          </Link>
                          <Link
                            href="/admin/reports"
                            className="flex items-center w-full px-3 py-2 text-left hover:bg-amber-50/50 dark:hover:bg-amber-900/80 rounded transition-colors duration-200 text-amber-800 dark:text-amber-200 group"
                          ...
                          >
                            ...
                            <span className="font-medium text-amber-800 dark:text-amber-200">Reports</span>
                          </Link>
                          <Link
                            href="/admin/users"
                            className="flex items-center w-full px-3 py-2 text-left hover:bg-amber-50/50 dark:hover:bg-amber-900/80 rounded transition-colors duration-200 text-amber-800 dark:text-amber-200 group"
                          ...
                          >
                            ...
                            <span className="font-medium text-amber-800 dark:text-amber-200">Users</span>
                          </Link>
                          <Link
                            href="/admin/vault"
                            className="flex items-center w-full px-3 py-2 text-left hover:bg-amber-50/50 dark:hover:bg-amber-900/80 rounded transition-colors duration-200 text-amber-800 dark:text-amber-200 group"
                          ...
                          >
                            ...
                            <span className="font-medium text-amber-800 dark:text-amber-200">Vaults</span>
                          </Link>
                        </div>
                      )}
                      ...
                      <div className="p-2">
                        ...
                        >
                          ...
                          <span className="font-medium text-gray-800 dark:text-white">Sign Out</span>
                        </button>
                      </div>
                    </div>
                  )}
                ...
                >
                  ...
                  <span className="text-foreground text-lg">Sign In</span>
                </button>
              )}
            </div>
          </div>
        ...
        <>
          ...
          >
            <div className="p-6 space-y-2" style={{ backgroundColor: theme === 'dark' ? '#3b4252' : 'white', color: theme === 'dark' ? '#eceff4' : 'inherit' }}>
              ...
              >
                <div className="px-4 py-2 text-gray-600 text-sm font-medium" style={theme === 'dark' ? { color: 'white' } : {}}>Theme</div>
                <div className="px-4">
              ...
              >
                <div className="px-4 py-2 text-gray-600 text-sm font-medium" style={theme === 'dark' ? { color: 'white' } : {}}>Select Language</div>
                <div className="grid grid-cols-2 gap-2 px-4">
                  {languages.map((language) => (
                    <button
                      key={language.code}
                      className={`flex items-center space-x-2 p-2 rounded ${selectedLanguage.code === language.code ? 'bg-primary/10 text-primary dark:text-primary-400' : language.code !== 'en' ? 'text-gray-400 dark:text-gray-500 cursor-not-allowed' : 'hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-800 dark:text-gray-200'}`}
                      onClick={() => {
                        if (language.code === 'en') {
                          setSelectedLanguage(language);
                        }
                    ...
                    >
                      <div className="w-8 h-6 relative">
                        <Image 
                          ...
                          width={32} 
                          height={24} 
                          className={`object-contain ${language.code !== 'en' ? 'opacity-50' : ''}`}
                        />
                      </div>
                      <span className="text-base">{language.name}</span>
                      {language.code !== 'en' && (
